...
本轮前后台联动评审使用现有前台的 5190 端口：`npm --prefix frontend run dev -- --port 5190`，后台仍为 5180。前台右上角“原型演示”切换正常与异常场景；个人中心提供模型接入和人民币用量。后台已发布专家的“预览已发布版本”打开现有前台；前台结果卡的诊断链接打开现有运行中心。链接只携带演示元信息，未新增独立原型入口或后端服务。
