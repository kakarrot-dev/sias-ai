...
export function UserModelAccess({ user, usage = false }: { user: ConsumerUser; usage?: boolean }) {
  ...
  return <div className="model-access-panel"><SourceSwitch source={source} change={setSource} /><p className="record-note">当前调用来源：{sourceLabels[selectedSource(user.id)]}（示例）。此处仅切换查看，不修改调用来源。</p>
    ...
    {usage ? <><KeyBalance access={access} /><KeyUsage access={access} /><Notice>全账号用量：未授权。OIDC 登录与专属 Key 均不代表已取得账号业务统计权限。</Notice></> : <>
      ...
      <Facts items={[["费用承担", source === 'platform' ? '平台额度' : '用户授权的 Key 对应额度'], ['授权方式', source === 'platform' ? '管理员配置平台中转站' : access.consentAt ? '用户主动提供并授权专属 Key（示例）' : '等待用户主动连接'], ['Key 标识', access.masked ?? '尚未连接'], ['Key 名称', access.keyName ?? '—'], ['Key ID', access.keyId ?? '—'], ['分组', access.group ? `${access.group.name} · ID ${access.group.id}` : '未提供'], ['协议', access.group?.protocol ?? '待确认'], ['有效期', access.expiresAt === null ? '未设置到期时间（示例）' : access.expiresAt ? formatTime(access.expiresAt) : '未提供'], ['最近模型查询', access.checkedAt ? formatTime(access.checkedAt) : '尚未查询'], ['用户授权时间', access.consentAt ? formatTime(access.consentAt) : source === 'platform' ? '不适用' : '尚未授权']]} />
